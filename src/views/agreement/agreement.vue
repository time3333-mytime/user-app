    ...
    <div class="page-container min-h-[100vh] py-[15px]">
      ...
      <div class="main-container py-[20px]">
        <van-button
          ...
          round
          class="m-t-20"
          @click="submitHandleByAlipay"
        >
          {{ isAddRenew ? "确认续约(支付宝支付)" : "保存" }}
        </van-button>
        <hr />
        <van-button
          v-if="isAllowEdit && isRenew"
          ...
          round
          class="m-t-20"
          @click="submitHandleByWechat"
        >
          {{ isAddRenew ? "确认续约（微信支付）" : "保存" }}
        </van-button>
        <!--      确认签约-->
        <van-button
...
// 续约,修改
const submitHandleByAlipay = async () => {
  console.log("确认签约");
  try {
    // 租期时长不能小于支付方式月份长度
    ...
    const { data } = await ToPay(agreementDetailInfo.value.id);
    agreementDetailInfo.value.status = 1;
    await saveOrUpdateAgreement(agreementDetailInfo.value);
    router.push(`/payment?qrcodeUrl=${encodeURIComponent(data.valueOf())}`);
  } catch (error) {
    console.log(error);
  }
};
const submitHandleByWechat = async () => {
  console.log("确认签约");
  try {
    // 租期时长不能小于支付方式月份长度
    let leaseTermMonthCount =
